...
class RoomStore:
    ...
    async def ping(self) -> None:
        try:
            await _resolve(self.redis.ping())
    ...
    async def create_room(
        ...
        *,
        ...
        await _resolve(self.redis.sadd(ROOM_INDEX_KEY, normalized))
    ...
    async def get_state(self, room_id: str) -> BackendState | None:
        encoded = _decode_text(await _resolve(self.redis.get(room_state_key(room_id))))
        if encoded is None:
            return None
    ...
    async def set_state(self, room_id: str, state: BackendState) -> None:
        ...
        await _resolve(self.redis.set(room_state_key(room_id), encoded, ex=self.ttl_seconds))
    ...
    async def get_metadata(self, room_id: str) -> dict[str, object] | None:
        encoded = _decode_text(await _resolve(self.redis.get(room_metadata_key(room_id))))
        if encoded is None:
            return None
    ...
    async def set_metadata(self, room_id: str, metadata: dict[str, object]) -> None:
        ...
        await _resolve(self.redis.set(room_metadata_key(room_id), encoded, ex=self.ttl_seconds))

    async def refresh_ttl(self, room_id: str) -> None:
        await _resolve(self.redis.expire(room_state_key(room_id), self.ttl_seconds))
        await _resolve(self.redis.expire(room_metadata_key(room_id), self.ttl_seconds))

    async def has_room(self, room_id: str) -> bool:
        return await self.get_state(room_id) is not None

    async def room_ids(self) -> tuple[str, ...]:
        members = cast(Any, await _resolve(self.redis.smembers(ROOM_INDEX_KEY)))
        return tuple(sorted(_decode_text(member) or "" for member in members))

    async def room_count(self) -> int:
        count = await _resolve(self.redis.scard(ROOM_INDEX_KEY))
    ...
    async def expire_empty_room(self, room_id: str, *, has_connections: bool) -> bool:
        if has_connections:
            ...
            return False
        normalized = normalize_room_id(room_id)
        await _resolve(self.redis.delete(room_state_key(normalized), room_metadata_key(normalized)))
        await _resolve(self.redis.srem(ROOM_INDEX_KEY, normalized))
        return True

    async def set_token_lookup(self, room_id: str, token_hash: str) -> None:
        normalized = normalize_room_id(room_id)
        await _resolve(self.redis.set(room_token_key(normalized, token_hash), normalized, ex=self.ttl_seconds))

    async def get_token_room_id(self, room_id: str, token_hash: str) -> str | None:
        return _decode_text(await _resolve(self.redis.get(room_token_key(room_id, token_hash))))

    async def is_pin_blocked(self, room_id: str, fingerprint: str) -> bool:
        normalized = normalize_room_id(room_id)
        value = await _resolve(self.redis.get(room_pin_block_key(normalized, fingerprint)))
    ...
    async def record_failed_pin_attempt(
        ...
        *,
        ...
        attempts = int(cast(int, await _resolve(self.redis.incr(attempts_key))))
        if attempts == 1:
            await _resolve(self.redis.expire(attempts_key, attempt_window_seconds))

        if attempts >= max_attempts:
            await _resolve(self.redis.set(block_key, "1", ex=block_seconds))
            return True
        return False

    async def acquire_tick_lock(self, room_id: str, owner: str, ttl_seconds: int = config.ROOM_TICK_LOCK_SECONDS) -> bool:
        normalized = normalize_room_id(room_id)
        acquired = await _resolve(self.redis.set(room_tick_lock_key(normalized), owner, ex=ttl_seconds, nx=True))

...
@pytest.fixture
def checkin_websocket_client(monkeypatch: pytest.MonkeyPatch) -> Generator[tuple[TestClient, FakeRedis], None, None]:
    redis = FakeRedis()
...
def receive_json_with_timeout(websocket):
    ...
    try:
        ...
        pytest.fail("timed out waiting for checkin state snapshot")
    finally:
        signal.setitimer(signal.ITIMER_REAL, 0)
        signal.signal(signal.SIGALRM, previous_handler)


def receive_checkin_snapshot(websocket, expected_last_id: str) -> dict[str, object]:
    for _ in range(16):
        update = receive_json_with_timeout(websocket)
        data = update["data"]
        assert isinstance(data, dict)
        checkins = data.get("checkins")
        assert isinstance(checkins, list)
        if checkins and isinstance(checkins[-1], dict) and checkins[-1].get("id") == expected_last_id:
            return update
    pytest.fail(f"checkin snapshot ending with {expected_last_id} was not received")


def test_checkin_commands_fan_out_as_full_state_snapshots(
    checkin_websocket_client: tuple[TestClient, FakeRedis],
):
    client, redis = checkin_websocket_client
    token_a = create_room(client, "CHECKIN")
    joined = client.post("/api/rooms/CHECKIN/join", json={"pin": "2468"})
    assert joined.status_code == 200
    token_b = str(joined.json()["token"])

    with client.websocket_connect(f"/ws/CHECKIN?token={token_a}") as ws_a:
        with client.websocket_connect(f"/ws/CHECKIN?token={token_b}") as ws_b:
            ws_a.receive_json()
            ws_b.receive_json()
            ...
            update_a = receive_checkin_snapshot(ws_a, "check_013")
            update_b = receive_checkin_snapshot(ws_b, "check_013")

    stored = json.loads(redis.values[room_state_key("CHECKIN")])
    ...
    assert stored["checkins"] == data_a["checkins"]
    assert [checkin["id"] for checkin in stored["checkins"]] == [f"check_{index:03d}" for index in range(2, 14)]
    assert all(set(checkin) == {"id", "kind", "text"} for checkin in stored["checkins"])

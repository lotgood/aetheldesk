...
| State authority | FastAPI owns authoritative room state. Standalone Redis stores the canonical JSON state outside explicit test mode; the in-memory path is restricted to pytest or `AETHELDESK_ENV=test`. |
...
| Redis policy | Redis is mandatory outside explicit test mode. The supported topology is standalone Redis (the bundled Docker path), not Redis Cluster: fenced Lua mutations atomically validate the room generation and refresh state, metadata, and token-set TTLs while touching the shared room index where required. The in-memory path is restricted to pytest or `AETHELDESK_ENV=test`. Redis outages are normalized to the existing HTTP `503 {"detail":"redis unavailable"}` and WebSocket `1011 service unavailable` operational contracts. |
...
| `/ws/{room_id}` | WebSocket | `room_id` path segment plus required `token` query parameter. The token must authorize the normalized room id and its current `room_instance_id`. Client URL shape is `/ws/${ROOM_ID}?token=${encodeURIComponent(token)}`. | Waits for Redis subscription readiness, binds the socket to one room generation, then sends the canonical `{"type":"state","data": BackendState}` snapshot. Inbound JSON-object messages use generation- and revision-fenced mutation before broadcasting full snapshots. | Missing token, invalid room id, bad token, missing room/state, or a replaced room generation close with code `1008` and reason `authentication failed`. Redis authorization, subscription, fanout, or mutation failures close with code `1011` and reason `service unavailable`. Malformed or non-object inbound JSON is logged and ignored. | `backend/main.py` `WS_AUTH_CLOSE_CODE`, `WS_OPERATIONAL_CLOSE_CODE`; `backend/websocket_handler.py` `ws_endpoint()`; `frontend/src/room-websocket.js` `wsUrl()` and close handler | `tests/test_room_auth.py::test_websocket_requires_valid_token`; `tests/test_websocket_redis.py`; `tests/test_frontend_static.py::test_room_auth_uses_tokenized_websocket_and_generic_korean_rejection`. |
...
- Endpoint: `/ws/{room_id}`.
- Query parameter: `token` is required. The backend scopes issued opaque-token hashes to the current room instance and binds every accepted socket to that exact `room_instance_id`.
...
- Operational close: code `1011`, reason `service unavailable`, used when Redis becomes unavailable during authorization, subscription, fanout, or message handling. A Pub/Sub consumer that fails after readiness closes its local sockets so browser reconnect performs a fresh canonical sync.
...
- Ongoing successful server messages: full-state snapshots shaped as `{"type":"state","data": BackendState}` after accepted client mutations, scheduler ticks, or Redis Pub/Sub fanout. Every local fanout targets only sockets bound to the canonical generation; mismatched sockets close with `1008` without receiving replacement state.
- Subscription handoff: a worker waits for the Redis `SUBSCRIBE` acknowledgement before canonical sync and socket registration. The post-subscribe canonical read covers earlier events while the active subscriber covers later events, eliminating the initial snapshot/subscription gap.
...
|---|---|---|---|---|
| Room registry | `aetheldesk:rooms` | `backend.room_store.ROOM_INDEX_KEY` | Shared room-id set used for scheduler discovery and capacity. Expired entries are pruned only after both canonical state and metadata are absent; one-sided state/metadata skew stays counted but is not tickable, preserving fail-closed auth. The standalone-Redis creation Lua script atomically checks the cap, creates state and metadata, clears the prior token set and tick lease, and registers the id. | `tests/test_room_store.py`; `tests/test_scheduler.py`. |
| State | `aetheldesk:room:{ROOM_ID}:state` | `room_state_key(room_id)` | Stores JSON-encoded `BackendState`; this is the canonical Redis room-state value. State CAS and tick commit require the expected `room_instance_id` and revision, then refresh state, metadata, and the optional token-set TTL in the same Lua transaction. | `tests/test_redis_contract.py`; `tests/test_room_store.py`; `tests/test_websocket_redis.py`. |
...
| Tick lock | `aetheldesk:room:{ROOM_ID}:tick-lock` | `room_tick_lock_key(room_id)` | Redis `TIME` fences an `NX` lease by logical second. `running` blocks overlap; `done` blocks duplicate work in the same second. Tick state is committed only when the lease, expected state `revision`, and room generation all match; same-id recreation deletes the previous lease. `last_tick_slot` applies elapsed Redis seconds after scheduler gaps without double-decrementing a same-slot message reconciliation. | `tests/test_redis_contract.py`; `tests/test_scheduler.py`; `tests/test_websocket_redis.py`. |
| Event channel | `aetheldesk:room:{ROOM_ID}:events` | `room_events_channel(room_id)` | Redis Pub/Sub channel for full-state event snapshots. Raw Redis transport errors normalize to `RedisUnavailable`; consumers expose subscription readiness and canonical-sync after acknowledgement. | `tests/test_redis_contract.py`; `tests/test_event_bus.py`; `tests/test_websocket_redis.py`. |

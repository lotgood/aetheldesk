...
| External frontend dependencies | The YouTube iframe API remains allowed. Vite should serve local build assets. Google Fonts should be self-hosted or given a local fallback later. |
...
|---|---|---|
| Decision record and route matrix | `.omo/plans/project-wide-improvement-plan.md`; `backend/main.py`; `backend/frontend_routes.py`; `backend/room_routes.py`; `backend/websocket_handler.py`; `frontend/lobby.html`; `frontend/app.js` | `tests/test_backend_routes.py`; `tests/test_room_auth.py`; `tests/test_frontend_static.py`; e2e room tests. |
...
| Frontend storage contract | `frontend/src/storage.js`; `frontend/lobby.js`; `frontend/src/music-youtube.js`; `frontend/scenes.js` | `tests/test_frontend_static.py`; `tests/e2e/test_room_pin.py`; `tests/e2e/test_room_sync.py`. |

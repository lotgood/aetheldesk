...
| `intent` | `RoomIntent` | Shared room focus plan foundation: `{"goal": string, "tasks": RoomTask[], "active_task_id": string or None}`; default empty goal, no tasks, no active task. Task ids are 8-24 URL-safe chars, task text is 1-80 chars, max 8 tasks, and goal text is max 120 chars. |
| `checkins` | `RoomCheckIn[]` | Bounded explicit room check-in foundation; default `[]`. Entries are `{"id": string, "kind": "ready" or "progress" or "done", "text": string}` with max 12 entries and max 80 text chars. This is not inferred online presence and does not contain token, PIN, or author data. |
| `scene` | `"sky"`, `"city"`, `"beach"`, or `"forest"` | Room scene foundation; default `"sky"`. Existing `localStorage.scene` remains a frontend preference key and does not change Redis/WebSocket key contracts. |
| `ambience` | `AmbienceState` | Shared ambient-sound settings foundation: `{"enabled": bool, "layers": {"rain": int, "wind": int, "brown_noise": int}}`; volumes are `0..100`, default disabled with all volumes `0`. |
| `metrics` | `RoomMetrics` | Ephemeral room recap foundation: `{"focus_seconds": int, "sessions_completed": int, "tasks_completed": int}`; non-negative ints, default all `0`, stored only in current room state and expired by normal room lifecycle. |

Accepted client mutation message types are handled in `backend/state.py`: `time_override`, `focus_toggle`, `focus_pause`, `focus_cancel`, `set_duration`, `skip_break`, `music_play`, `music_pause`, `music_skip`, `location`, `intent_set_goal`, `intent_add_task`, `intent_update_task`, `intent_toggle_task`, `intent_select_task`, `intent_delete_task`, `intent_clear_completed`, `checkin_add`, `checkin_clear`, `scene_select`, `ambience_set_enabled`, and `ambience_set_layer`. Tests in `tests/test_backend_state.py`, `tests/test_websocket_redis.py`, `tests/test_event_bus.py`, `tests/test_intent_messages.py`, `tests/test_intent_websocket.py`, `tests/test_checkin_messages.py`, `tests/test_checkin_websocket.py`, `tests/test_scene_messages.py`, `tests/test_ambience_messages.py`, and `tests/test_metrics_state.py` assert JSON-serializable defaults, compatibility exports, state transitions, Redis persistence, room-local metrics, and full-state snapshot fanout.

The feature foundation fields `intent`, `checkins`, `scene`, `ambience`, and `metrics` are part of the canonical full-state snapshot schema as of the feature-roadmap contract foundation. Dedicated client mutation commands currently exist for `intent`, `checkins`, `scene`, and `ambience`; `metrics` is mutated internally by the timer tick path and task completion reducer, with no client command and no storage outside the canonical room state.
...
| `localStorage` | `scene` | `frontend/scenes.js` `activeScene`, `applyScene()`, and `createSceneController()` | Scene selection is persisted in `localStorage` under `scene` only as a startup preference. After room connection, the server `BackendState.scene` snapshot is authoritative. | `tests/test_frontend_static.py::test_frontend_persists_only_playlist_and_scene_local_storage_keys`; `tests/e2e/test_room_scene_shared.py`. |
...
| `BackendState` contract | `backend/state.py`; `backend/state_features.py`; `backend/main.py`; `backend/room_store.py`; `backend/state_codec.py`; `backend/event_bus.py`; `backend/scheduler.py` | `tests/test_backend_state.py`; `tests/test_feature_state_contract.py`; `tests/test_state_codec.py`; `tests/test_room_store.py`; `tests/test_websocket_redis.py`; `tests/test_event_bus.py`; `tests/test_intent_messages.py`; `tests/test_checkin_messages.py`; `tests/test_scene_messages.py`; `tests/test_ambience_messages.py`; `tests/test_metrics_state.py`. |
| Frontend storage contract | `frontend/src/storage.js`; `frontend/lobby.js`; `frontend/src/music-youtube.js`; `frontend/scenes.js`; `frontend/src/room-controller.js`; `frontend/src/room-intent.js`; `frontend/src/room-checkins.js`; `frontend/src/ambience-audio.js`; `frontend/src/room-recap.js` | `tests/test_frontend_static.py`; `tests/test_frontend_intent_static.py`; `tests/test_frontend_checkins_static.py`; `tests/test_frontend_scene_static.py`; `tests/test_frontend_ambience_static.py`; `tests/test_frontend_recap_static.py`; `tests/e2e/test_room_pin.py`; `tests/e2e/test_room_sync.py`; feature e2e tests. |
